...
\iffalse
    The subroutine $\mathtt{SelectMinimalClauses}$ for the technique is outlined in Figure~\ref{fig:minimal}.
    \begin{figure}[h]
        \mbox{}\hrulefill \vspace{-.6em}
        \small
        \begin{program}
            \>  {\bf \textit{SelectMinimalClauses}}\\
            \> \> \INPUT: The matrix $\phi(X,Y)$ and selection relation $\psi(X,S)$\\
            \> \> \OUTPUT: An assignment $\tau'$ selecting a minimal set of clauses\\
            \> \> \BEGIN\\
            \> \> 01 \> \> \DO\\
            \> \> 02 \> \> \> $\tau'$ := $\psi.\mathrm{model}$ for variables in $X$;\\
            \> \> 03 \> \> \> $C_L$ := $\bigvee_{C \in \phi|_{\tau'}}\neg s_C$;\\
            \> \> 04 \> \> \> $\mu$ := $\bigwedge_{C \notin \phi|_{\tau'}}\neg s_C$;\\
            \> \> 05 \> \> \> $\psi$ := $\psi \wedge C_L$;\\
            \> \> 06 \> \> \WHILE $\mathtt{SAT}(\psi|_{\mu}) = \top$;\\
            \> \> 07 \> \> \RETURN $\tau'$; \\
            \> \> \END
        \end{program}
        \vspace{-1.2em} \mbox{}\hrulefill \caption{\small Subroutine: Selecting a minimal set of clauses.}
        \label{fig:minimal}
    \end{figure}
\fi
...
\iffalse
    \begin{example}\label{ex:minimal}
        ...
        The first tried assignment $\tau_1=\neg e_1 \neg e_2 \neg e_3$ selects $C_1$, $C_2$.
        The set of selected clauses is made minimal as follows.
        The subroutine $\mathtt{SelectMinimalClauses}$ conjoins the selection relation with a learnt clause $(\neg s_1 \vee \neg s_2)$, which prevents clauses $C_1$ and $C_2$ being simultaneously selected again.
        The satisfiability of the selection relation $\psi \wedge (\neg s_1 \vee \neg s_2)$ is tested under the assumption $\neg s_3 \neg s_4$, which avoids the originally deselected clause $C_3$ and $C_4$ being selected.
        The formula is satisfied by the assignment $\tau_2=\neg e_1 e_2 \neg e_3$, which only selects $C_1$.
        By repeating the above operation again, the assignment $\tau_5=e_1 \neg e_2 e_3$ is found, since it selects no clause.
        The conditional satisfying probability under $\tau_5$, which equals $1$, is derived without invoking the weighted model counter.
        Compared to Example~\ref{ex:basic} where five assignments over $X$ were explored, the algorithm with the minimal clause selection technique finds the optimal assignment $\tau_5$ with at most two SAT calls, thus greatly improves the computation efficiency.
    \end{example}
\fi
