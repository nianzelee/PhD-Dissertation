...
a set $\pcf{\pf}{\as_1}$ of clauses is selected.
...
The above idea gives rise to~\cref{alg:erssat} for solving E-MAJSAT formulas.
...
we can safely assert pure $X$ variables without missing the optimal solution.
...
The blocking clause $C_L$ derived from the containment-learning technique is then conjoined with $\select$ to prevent clauses in $\pcf{\pf}{\as}$ from being simultaneously selected again.
...
\begin{example}
    ...
    The learnt clause $C_L=(\lnot s_1 \lor \lnot s_2)$ is conjoined with $\select$ to prevent $C_1$ and $C_2$ from being selected simultaneously again.
    ...
    and the learnt clause $C_L=(\lnot s_1)$ is conjoined with $\select$ to prevent $C_1$ from being selected again.
    ...
    and the learnt clause $C_L=(\lnot s_4)$ is conjoined with $\select$ to prevent $C_4$ from being selected again.
    ...
    and the learnt clause $C_L=(\lnot s_3)$ is conjoined with $\select$ to prevent $C_3$ from being selected again.
...
Starting from a set $\pcf{\pf}{\as}$ of initially selected clauses,
...
A learnt clause $C_S=\bigvee\limits_{C\in\pcf{\pf}{\as}}\lnot\sv{C}$ is conjoined with the selection relation $\select(X,S)$ to avoid clauses in $\pcf{\pf}{\as}$ from being selected simultaneously again.
A SAT solver is invoked to solve $\select\land C_S$ under an assumption $\mu=\bigwedge\limits_{C\notin\pcf{\pf}{\as}}\lnot\sv{C}$.
The assumption $\mu$ prevents originally deselected clauses from being selected.
...
\begin{example}
    ...
    which prevents clauses $C_1$ and $C_2$ from being simultaneously selected again.
    ...
    which avoids the originally deselected clause $C_3$ and $C_4$ from being selected.
    ...
    the algorithm with the minimal clause selection technique
    finds the optimal assignment from $\as_1$ with two additional SAT calls,
...
The subroutine $\texttt{RemoveSubsumedClauses}$ simplifies the set $\pcf{\pf}{\as}$ of selected clauses by removing subsumed clauses.
A clause $C$ is removed from the set $\pcf{\pf}{\as}$ of selected clauses if $C^Y$ is subsumed by other selected sub-clauses.
...
\begin{example}
    ...
    and the learnt clause $C_L=(\lnot \sv{1})$ is conjoined with $\select$ to prevent $C_1$ from being selected again.
...
The subroutine $\texttt{DiscardLiterals}$ for the technique to discard literals from a learnt clause is outlined in~\cref{alg:erssat-partial}.
...
the proposed algorithm can be easily modified to solve \textit{approximate SSAT} by returning the greatest encountered lower bound upon timeout, as explained in~\cref{ex:erssat-basic}.
...
whose sizes are typically much smaller than that of the original matrix.
...
A BDD is then constructed based on the resultant circuit.
Weighted model counting of the original formula is done by traversing the BDD~\cite{Darwiche2002KnowledgeCompilation}.
We use the well-developed BDD package \cudd~\cite{CUDD},
which caches BDD nodes during building BDDs for different formulas.
Thus the formula caching effect as well as garbage collection,
which removes unreferenced nodes, is automatically achieved by \cudd.
